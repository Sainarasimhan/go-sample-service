...
func main() {
	...
	var cfg *config.Cfg
	{
		cfg, err = config.Init()
		if err != nil {
			lg.Fatal("Config error", err) // Service will exit on any config errors
		} else {
			c := cfg.Sample
	...
	var otTracer trace.Tracer
	{
		...
		exporter, err := stdout.NewExporter(stdout.Options{Writer: lg.Writer(), PrettyPrint: false})
		if err != nil {
			lg.Fatal(err)
		}
	...
	// Setup Metrics
	{
		...
		*/
		...
		if err != nil {
			lg.Panicf("failed to initialize metric stdout exporter %v", err)
		}
		defer pusher.Stop()
	...
	var db repo.Repository
	{
		...
		if err != nil {
			lg.Fatal("Error creating repo", err) // Service exits on any error creating repo
		} else {
			defer db.Close()
		}
	}
	...
	var g group.Group
	{
		...
		if err != nil {
			lg.Fatal("main() - HTTP(debug) \t", err)
		}
		g.Add(func() error {
			lg.Info("Address", *debugAddr)("starting HTTP(debug) server ...")
	...
	}
	{
		...
		if err != nil {
			lg.Fatal("main() - HTTP \t ", err)
		}
		g.Add(func() error {
			lg.Info("Address", *httpAddr)("starting HTTP server ...")
	...
	}
	{
		...
		if err != nil {
			lg.Fatal("transport gRPC \t", err)
		}
		g.Add(func() error {
			lg.Info("Address", *grpcAddr)("starting gRPC server ...")
			...
			if grpcCfg.Secure {
				...
				if err != nil {
					lg.Fatal("transport grpc secureStartup:", err)
				}
	...
	}
	{
		...
		if *cpuprofile != "" {
			...
			lg.Debug()("Cpu Profiling Started")
